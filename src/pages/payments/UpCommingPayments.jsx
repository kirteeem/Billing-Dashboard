...
function StatCard({ title, value, variant }) {
  ...
  return (
    <div
      className={`w-full h-[106px] rounded-xl px-6 py-4 flex items-center justify-between border border-[#E5EAF0]
        ${isTotal ? "bg-gradient-to-r from-[#DEF3FD] to-[#F5FDFF]" : "bg-white"}
      `}
    >
      {/* LEFT */}
      <div>
        <div className="text-sm text-[rgba(130,138,150,1)]">
          {title}
        </div>

        <div
          className={`mt-1 font-semibold ${
            isTotal ? "text-[28px]" : "text-[24px]"
          }`}
        >
          {value}
        </div>
...
function OverdueSection() {
  return (
    <SectionWrapper title="Overdue" icon={<OverdueIcon />}>
      <PaymentRow
  icon={<AuthOIcon />}
  name="Auth0"
...
<PaymentRow
  icon={<img src={bunnyLogo} alt="Bunny CDN" className="w-6 h-6" />}
  name="Bunny CDN"
  date="Due Feb 5, 2026"
  amount="$195"
  status="2 days"
...
function SectionWrapper({ title, icon, children }) {
  return (
    <div className="w-full border border-[#E5EAF0] rounded-xl bg-white px-6 py-6">
      {/* HEADER */}
  ...
  style={{
    ...
    fontSize: "18px",
    lineHeight: "28px",
    color: "rgba(220, 40, 40, 1)", // for Overdue
  }}
>
...
function PaymentRow({ icon, name, date, amount, status, theme }) {
  ...
  return (
    <div
      className={`h-[86px] rounded-xl border px-6 flex items-center justify-between ${themeMap[theme]}`}
    >
      {/* LEFT */}
  ...
  {date}
      ...
      <div className="flex items-center gap-6">
        <div className="text-right">
          <div className="font-semibold text-[#0F1729]">{amount}</div>
          <div
  style={{
    fontFamily: "Rethink Sans",
    fontWeight: 500,
...
>
  {status}
       ...
       <button className="h-10 px-4 rounded-lg bg-[#0EA5E9] hover:bg-[#38BDF8] transition-colors duration-200 text-white text-sm font-medium">
  Pay Now
</button>
